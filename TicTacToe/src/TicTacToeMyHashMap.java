/**
 * Builds a HashMap of TicTacToe winners using my hash function and analyzes it.
 * 
 * Maxwell Dodson
 * DE CS II
 * 3/15/20
...
public class TicTacToeMyHashMap  {
	...
	HashMap<NewString, Boolean> map;

	/**
	 * Instantiates a new TicTacToeMyHashMap that puts TicTacToe
	 * winners in a HashMap using my hash function
	 *
	 */
	public TicTacToeMyHashMap() {
		 map = new HashMap<NewString, Boolean>();
		 File in = new File(TicTacToe.WINNERS); // Attempt to find winners file
			try {
				...
				while (input.hasNextLine()) { // Read in lines of input file
					...
					// Put winners in the map using my hash function in NewString
					map.put(new NewString(line), true);
				}
    ...
    public void listStats() throws NoSuchFieldException, IllegalAccessException {
    	...
    	for (int i=0; i<table.length; i++) { // Iterate through Objects in the HashMap table
    		if (table[i] != null) { // Verify this space in the HashMap has been filled
    			int elements = 0;
    			Field next;
    			Object node = table[i];
